...
func fetchVkCreds(ctx context.Context, link string, streamID int, captchaResultChan chan string, getCaptchaMode func() string, emitCaptchaRequest func(mode, redirectURI, sessionToken string)) (string, string, []string, error) {
	...
	for attempt := 0; attempt < vkCredentialAttemptLimit; attempt++ {
		...
		user, pass, addrs, err := getTokenChain(ctx, link, streamID, creds, jar, captchaResultChan, getCaptchaMode, emitCaptchaRequest)
...
func getTokenChain(ctx context.Context, link string, streamID int, creds VKCredentials, jar tlsclient.CookieJar, captchaResultChan chan string, getCaptchaMode func() string, emitCaptchaRequest func(mode, redirectURI, sessionToken string)) (string, string, []string, error) {
...
func requestWebViewCaptcha(streamID int, captchaErr *VkCaptchaError, mode string, timeout time.Duration, captchaResultChan chan string, emitCaptchaRequest func(mode, redirectURI, sessionToken string)) (string, error) {
